                ...
                <div class="row col-12 page-titles" id="div_maincourse">
                  <div class="col-md-12 card">
                    <div class="card-body">
                      <div class="col-md-12">
                        <?php if($btn_add=="1"){ ?>
                          <button name="add_button" id="add_button" class="btn btn-outline-info add_button float-right" data-toggle="modal" data-target="#modal-default"><i class="mdi mdi-plus-box-outline"></i> <?php echo label('adddivision'); ?></button>
                        <?php } ?>
                          ...
                          <table id="myTable" width="100%" class="table table-bordered table-striped">
                            <thead>
                              <tr>
                                  ...
                                  <th width="5%"></th>
                                  <th width="10%"><center><?php echo label('div_code'); ?></center></th>
                                  <th width="45%"><center><?php echo label('div_name'); ?></center></th>
    ...
    <div class="modal fade bs-example-modal-lg" id="modal-default" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
      <div class="modal-dialog modal-lg">
          <div class="modal-content">
              ...
              <form method="post" id="division_form" autocomplete="off" name="division_form" enctype="multipart/form-data"  class="form-horizontal" role="form">
              <div class="modal-body row">
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="div_code"><b style="color: #FF2D00">*</b><?php echo label('div_code'); ?>:</label>
                    <input type="text" id="div_code" name="div_code" class="form-control" onchange="onchkcode(this.value)" required> 
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="div_name_th"><b style="color: #FF2D00">*</b><?php echo label('div_name')." TH"; ?>:</label>
                    <input type="text" id="div_name_th" name="div_name_th" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="div_name_en"><b style="color: #FF2D00">*</b><?php echo label('div_name')." EN"; ?>:</label>
                    <input type="text" id="div_name_en" name="div_name_en" class="form-control" required> 
                  </div>
                </div>
              </div>
    ...
    <script type="text/javascript">
           ...
           function onchkcode(value){
              ...
              $.ajax({
                ...
                method:"POST",
                data:{code:value,field_name:"LMS_DIVISION",com_id:com_id},
                dataType:"json",
      ...
      function fetch_data_main(page_num)
         {
            ...
            "scrollX": true,
                "ajax": {
                    url : '<?=base_url()?>index.php/manage/fetch_detail_division/',
                    type : 'GET',
                    data : {com_id:com_id}
                },
    ...
    $(document).ready(function() {
        ...
        $(document).on('submit', '#division_form', function(event){
              event.preventDefault(); 
                $.ajax({
                  url:"<?=base_url()?>index.php/manage/insert_division",
                  method:'POST',
         ...
         $(document).on('click', '.delete', function(){
            ...
            swal({
              ...
              if(isChk.value){
                $.ajax({
                    url:"<?=base_url()?>index.php/manage/delete_division_data",
                    method:"POST",
                    data:{div_id:div_id},
                    success:function(data)
                    {
           ...
           $('#add_button').click(function(){
                  ...
                  $('.modal-title').text('<?php echo label("adddivision"); ?>');
                  $('#division_form')[0].reset();
                  $('#operation').val("Add");
                  $('#com_id').val(com_id);
                  $('#div_code').attr('readonly', false);
            });
          $(document).on('click', '.update', function(){
            var div_id = $(this).attr("id");
            
            $.ajax({
              url:"<?=base_url()?>index.php/manage/update_division_data",
              method:"POST",
              data:{div_id:div_id},
              dataType:"json",
              success:function(data)
              {         
                  $("#modal-default").modal({backdrop: false});
                  $('.modal-title').text('<?php echo label("editdivision"); ?>');
                  $('#division_form')[0].reset();
                  $('#operation').val("Edit");
                  $('#div_code').attr('readonly', true);
                $('#div_id').val(data.div_id);
                $('#div_code').val(data.div_code);
                $('#div_name_en').val(data.div_name_en);
                $('#div_name_th').val(data.div_name_th);

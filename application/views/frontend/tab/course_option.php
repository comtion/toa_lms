            ...
            <?php 
                ...
                if($lang_select=="thai"){
                  ...
                  $passtxt = 'ผ่าน';
                  $failtxt = 'ไม่ผ่าน';
                  $cannotaccess = 'กรุณาทำส่วนหน้าให้เสร็จสิ้นก่อนจึงจะสามารถเข้าถึงส่วนถัดไปได้';
                  ...
                  $passtxt = 'Passed';
                  $failtxt = 'Failed';
                  $cannotaccess = 'Please complete previous section before be able to access next section';
                  ...
                  $passtxt = '合格';
                  $failtxt = '不合格';
                  $cannotaccess = '次のセクションを続けるため、前のセクションを完了する必要があります。';
                  $condition_msg = '(_coursename_)の前提条件をｸﾘｱしたら、このｺｰｽを勉強することになります';
                }
                  $num_loop_div = 0;
                if(isset($pretest_arr)&&count($pretest_arr)>0){
                      foreach ($pretest_arr as $key_pretest => $value_pretest) {
                        if(isset($value_pretest['question'])&&count($value_pretest['question'])>0){
                  ...
                  $status_update = $value_pretest['status_tc']!="3"?0:1;
                  $statustxt = isset($value_pretest['statustxt'])&&$value_pretest['statustxt']=="Pass"?$passtxt:$failtxt;
            ?>
                ...
                <div class="collapse" id="collapseExample_<?php echo $value_pretest['qiz_id']; ?>">
                  ...
                  <div class="card card-body" style="height: 550px;">
                      <div class="vtabs customvtab">
                          ...
                          <?php $numpretest = 0;
                                if($quiz_info!=""){ $numpretest++; ?>
                            <div class="tab-pane <?php if($numpretest==1){ ?>active show<?php } ?> card-body" id="quiz_detail" role="tabpanel">
                              <h4><?php echo $summarytxt; ?></h4>
                              <p>
                                <?php echo str_replace('../uploads/texteditor/', base_url().'/uploads/texteditor/',$quiz_info); ?>
                              </p>
                              <hr>
                              <button type="button" class="btn btn-outline-secondary float-right" onclick="onclickfirstquestion('<?php echo $div_first; ?>')"><?php if($value_pretest['status_tc']!="3"&&$value_pretest['endstatus']=="0"){ echo $qiz_starttxt; }else{ echo $chk_answer_label; } ?></button>
                            </div>
                          <?php }
                                $numpretest++;  ?>
                          ...
                          ?>
                            ...
                            <div class="tab-pane <?php if($numpretest==1){ ?>active show<?php } ?> card-body" id="quiz_<?php echo $value_pretest['qiz_id']."_".$numloop; ?>" role="tabpanel">
                              ...
                              <textarea class="form-control" <?php if($value_pretest['endstatus']=='1'){ echo "disabled"; }?> id="txt_answer_pretest_<?php echo $value_ques['ques_id']; ?>" name="txt_answer_<?php echo $value_ques['ques_id']; ?>" onkeyup="onselectVal('pretest','<?php echo $value_ques['ques_id']; ?>',this.value)" maxlength="10000" rows="5" <?php if(isset($value_ques['tc'])&&count($value_ques['tc'])>0&&$value_ques['tc_isSavescore']=="1"){ ?> readonly <?php } ?>><?php echo isset($value_ques['tc']['tc_answer'])?$value_ques['tc']['tc_answer']:""; ?></textarea>
                                <?php }else{ ?>
                                  <input type="text" class="form-control" <?php if($value_pretest['endstatus']=='1'){ echo "disabled"; }?> id="txt_answer_pretest_<?php echo $value_ques['ques_id']; ?>" onkeyup="onselectVal('pretest','<?php echo $value_ques['ques_id']; ?>',this.value)"  maxlength="255" name="txt_answer_<?php echo $value_ques['ques_id']; ?>" value="<?php echo isset($value_ques['tc']['tc_answer'])?$value_ques['tc']['tc_answer']:""; ?>" <?php if(isset($value_ques['tc'])&&count($value_ques['tc'])>0&&$value_ques['tc_isSavescore']=="1"){ ?> readonly <?php } ?>>
                                <?php } ?>
                              <?php }else{
...
strip_tags($arr_choice[$choicenum]['name_choice'],"<label>")
                                        $tc_answer = isset($value_ques['tc']['tc_answer'])?$value_ques['tc']['tc_answer']:"";
                                        if($mul_c1!=""){
                              ?>
                              ...
                              <?php     }*/
                                ...
                                <div class="row">
                                  ...
                                  <div class="col-10 text-right">
                                    ...
                                    <button type="button" onclick="click_next('quiz_<?php echo $value_pretest['qiz_id']."_".$numnext; ?>','<?php echo $value_ques['ques_id'];?>','<?php echo $value_pretest['qiz_id']; ?>','pretest','<?php echo $value_pretest['endstatus']; ?>')"  class="btn btn-outline-secondary"><?php echo $m_next; ?> <i class="mdi mdi-chevron-right"></i></button>
                                  <?php } ?>
            ...
            <?php if(isset($lesson_arr)&&count($lesson_arr)>0){ 
                    $status_update = $lesson_status!=count($lesson_arr)?0:1;
            ?>
              <div class="container-fluid p-0 mb-3" id="less_div" <?php if(count($pretest_arr)>0){ ?>onclick="showAlert('less_div')"<?php } ?>>
                <a href="#" id="lessonheader" class="btn btn-block <?php if($lesson_status==count($lesson_arr)){ ?>imat-red-bg btn-danger<?php } ?> waves-effect waves-light rounded-0 text-left onclickrechk div_header_tab<?php echo $num_loop_div;$num_loop_div++; ?> <?php if($loop_run==0){echo "disable";} ?>" type="button" data-toggle="collapse" data-target="#collapseExample_lesson" id="lessonlink" data-statustc="<?php echo $status_update; ?>" aria-expanded="false" <?php if($lesson_status!=count($lesson_arr)){ ?>style="background-color: #95a5a6;color: #ecf0f1;border-color: #95a5a6;"<?php } ?> aria-controls="collapseExample_lesson">
...
*/
                            ...
                            <div class="tab-pane card-body" id="lesson_<?php echo $value_lesson['les_id']; ?>" role="tabpanel">
                          <?php 
                          ...
                          ?>
                              ...
                              ?>                                                  
                                <div class="col-md-4">
                                    <br>
                                    <div onclick="onplayer_video_cos('<?php echo $value_med['type']; ?>','<?php echo preg_replace( "/\r|\n/", "",$value_med['video']); ?>','<?php echo $value_med['id']; ?>','<?php echo $value_lesson['les_id']; ?>','<?php echo $value_lesson['les_isSeekbar']; ?>')" class="onplayer_video" style="width: 100%;height: 150px;background-image: url('<?php echo $thumbnail_med;?>');background-position: center;background-size:cover;display: flex;justify-content:center;align-items: center;cursor: pointer;">
                      ...
                      ?>
                      <div class="row">
                                      <div class="col-lg-12" id="div_scorm_ddd" style="height: 700px">
                                      ...
                                      <div class="col-lg-12" id="div_scorm_btn"><br>
            ...
            <?php if(isset($posttest_arr)&&count($posttest_arr)>0){
                      foreach ($posttest_arr as $key_posttest => $value_posttest) {
                        if(isset($value_posttest['question'])&&count($value_posttest['question'])>0){
                  ...
                  $status_update = $value_posttest['status_tc']!="3"?0:1;
                  $statustxt = isset($value_posttest['statustxt'])&&$value_posttest['statustxt']=="Pass"?$passtxt:$failtxt;
                  //$value_posttest['status_tc']!="3"&&
            ?>
                      ...
                      <div class="vtabs customvtab">
                          ...
                          <?php $numposttest = 0;
                            ...
                            <div class="tab-pane <?php if($numposttest==1){ ?>active show<?php } ?> card-body" id="quiz_detail" role="tabpanel">
                              ...
                              <hr>
                              <button type="button" class="btn btn-outline-secondary float-right" onclick="onclickfirstquestion('<?php echo $div_first; ?>')"><?php if($value_posttest['status_tc']!="3"&&$value_posttest['endstatus']=="0"){ echo $qiz_starttxt; }else{ echo $chk_answer_label; } ?></button>
                            </div>
                          <?php }
                                $numposttest++;  ?>
                          ...
                          ?>
                            ...
                            <input type="hidden" id="tc_save_posttest_<?php echo $value_ques['ques_id']; ?>" name="tc_save_posttest_<?php echo $value_posttest['qiz_id']; ?>[]" value="<?php echo isset($value_ques['tc'])&&count($value_ques['tc'])>0&&$value_posttest['status_tc']=="3"?1:0; ?>">
                            ...
                            <div class="tab-pane <?php if($numposttest==1){ ?>active show<?php } ?> card-body" id="quiz_<?php echo $value_posttest['qiz_id']."_".$numloop; ?>" role="tabpanel">
                              ...
                              <?php if($value_ques['ques_type']=="sub"||$value_ques['ques_type']=="sa"){ ?>
                                <?php if($value_ques['ques_type']=="sub"){ ?>
                              <textarea class="form-control" <?php if($value_posttest['endstatus']=='1'){ echo "disabled"; }?> id="txt_answer_posttest_<?php echo $value_ques['ques_id']; ?>" name="txt_answer_<?php echo $value_ques['ques_id']; ?>" onkeyup="onselectVal('posttest','<?php echo $value_ques['ques_id']; ?>',this.value)" maxlength="10000" rows="5" <?php if(isset($value_ques['tc'])&&count($value_ques['tc'])>0&&$value_ques['tc_isSavescore']=="1"){ ?> readonly <?php } ?>><?php echo isset($value_ques['tc']['tc_answer'])?$value_ques['tc']['tc_answer']:""; ?></textarea>
                                <?php }else{ ?>
                                  <input type="text" class="form-control" <?php if($value_posttest['endstatus']=='1'){ echo "disabled"; }?> id="txt_answer_posttest_<?php echo $value_ques['ques_id']; ?>" onkeyup="onselectVal('posttest','<?php echo $value_ques['ques_id']; ?>',this.value)"  maxlength="255" name="txt_answer_<?php echo $value_ques['ques_id']; ?>" value="<?php echo isset($value_ques['tc']['tc_answer'])?$value_ques['tc']['tc_answer']:""; ?>" <?php if(isset($value_ques['tc'])&&count($value_ques['tc'])>0&&$value_ques['tc_isSavescore']=="1"){ ?> readonly <?php } ?>>
                                <?php } ?>
                              <?php }else{
                              ...
                              <?php     }*/
                                ...
                                <div class="row">
                                  ...
                                  <div class="col-10 text-right">
                                    ...
                                    <button type="button" onclick="click_next('quiz_<?php echo $value_posttest['qiz_id']."_".$numnext; ?>','<?php echo $value_ques['ques_id'];?>','<?php echo $value_posttest['qiz_id']; ?>','posttest','<?php echo $value_posttest['endstatus']; ?>')"  class="btn btn-outline-secondary"><?php echo $m_next; ?> <i class="mdi mdi-chevron-right"></i></button>
                                  <?php } ?>
            ...
            <?php       }
                  ...
                  if(isset($survey_arr)&&count($survey_arr)>0){
                  ...
                  $status_update = $value_survey['status_tc']!="1"?0:1;
            ?>
            <div class="container-fluid p-0 mb-3" id="div_survey<?php echo $value_survey['sv_id']; ?>" onclick="showAlert('div_survey<?php echo $value_survey['sv_id']; ?>')">
          ...
          <?php         
                  ...
                  <script type="text/javascript">
                    var loop_number = parseInt('<?php echo $num_loop_div; ?>');
                    var numrechk = 0;
                    for(iloop = 0;iloop<loop_number;iloop++){
                      ...
                      if(iloop!=0){
                        if(status_tc==0){
                          ...
                          $('.div_header_tab'+iloop).off('click');
                        }else{

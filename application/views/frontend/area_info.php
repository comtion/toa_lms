...
<body class="fix-header fix-sidebar card-no-border">
                          ...
                          <table id="myTable" width="100%" class="table table-bordered table-striped">
                            <thead>
                              <tr>
                                  ...
                                  <th width="5%"></th>
                                  <th width="20%"><center><?php echo label('section_name'); ?></center></th>
                                  <th width="10%"><center><?php echo label('salearea_code'); ?></center></th>
                                  <th width="25%"><center><?php echo label('salearea_name'); ?></center></th>
    ...
    <div class="modal fade bs-example-modal-lg" id="modal-default" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
      <div class="modal-dialog modal-lg">
          <div class="modal-content">
              ...
              <form method="post" id="area_form" autocomplete="off" name="area_form" enctype="multipart/form-data"  class="form-horizontal" role="form">
              <div class="modal-body row">
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="section_id"><b style="color: #FF2D00">*</b><?php echo label('dep_name'); ?>:</label>
                    <select class="form-control select2" id="section_id" name="section_id" onchange="onchkcode($('#salearea_code').val())" style="width: 100%"></select>
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="salearea_code"><b style="color: #FF2D00">*</b><?php echo label('salearea_code'); ?>:</label>
                    <input type="text" id="salearea_code" name="salearea_code" onchange="onchkcode(this.value)" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="salearea_shot"><b style="color: #FF2D00">*</b><?php echo label('acronym'); ?>:</label>
                    <input type="text" id="salearea_shot" name="salearea_shot" maxlength="6" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="salearea_name_th"><b style="color: #FF2D00">*</b><?php echo label('salearea_name')." TH"; ?>:</label>
                    <input type="text" id="salearea_name_th" name="salearea_name_th" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="salearea_name_en"><b style="color: #FF2D00">*</b><?php echo label('salearea_name')." EN"; ?>:</label>
                    <input type="text" id="salearea_name_en" name="salearea_name_en" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-12">
                  <div class="form-group">
                    <label><?php echo label('detail'); ?>:</label>
                    <textarea class="form-control" rows="8" id="salearea_remark" name="salearea_remark"></textarea>
                  </div>
                </div>
              </div>
              <input type="hidden" id="operation" name="operation" value="Add">
              <input type="hidden" id="com_id" name="com_id">
              <input type="hidden" id="salearea_id" name="salearea_id">
    ...
    <script type="text/javascript">
           ...
           function onchkcode(value){
              ...
              $.ajax({
                ...
                method:"POST",
                data:{code:value,field_name:"LMS_AREA",com_id:com_id,val_2:section_id},
                dataType:"json",
      ...
      function fetch_data_main(page_num)
         {
            ...
            "scrollX": true,
                "ajax": {
                    url : '<?=base_url()?>index.php/manage/fetch_detail_area/',
                    type : 'GET',
                    data : {com_id:com_id}
                },
    ...
    $(document).ready(function() {
        $('.dropify').dropify();
        //$('#myTable').DataTable();
        $(document).on('submit', '#area_form', function(event){
              event.preventDefault(); 
                $.ajax({
                  url:"<?=base_url()?>index.php/manage/insert_area",
                  method:'POST',
         ...
         $(document).on('click', '.delete', function(){
            ...
            swal({
              ...
              if(isChk.value){
                $.ajax({
                    url:"<?=base_url()?>index.php/manage/delete_area_data",
                    method:"POST",
                    data:{salearea_id:salearea_id},
                    success:function(data)
                    {
                      if(data == "2"){
                        swal(
           ...
           $('#add_button').click(function(){
                  ...
                  $('.modal-title').text('<?php echo label("addarea"); ?>');
                  $('#area_form')[0].reset();
                  $('#operation').val("Add");
                  $('#com_id').val(com_id);
                  $('#salearea_code').attr('readonly', false);

                  $.ajax({
                        url: '<?=base_url()?>index.php/querydata/rechecksection',
                        type: 'POST',
                        data:{section_id:'',com_id:com_id},
                        success: function(data_group){
                          $('#section_id').html(data_group);
                          $('#section_id').val($("#section_id option:first").val()).trigger('change');
                        }
                  });
            });
          ...
          $(document).on('click', '.update', function(){
            ...
            var com_id = $('#com_id_search').val();
            $.ajax({
              url:"<?=base_url()?>index.php/manage/update_area_data",
              method:"POST",
              data:{salearea_id:salearea_id},
              dataType:"json",
              success:function(data)
              {         
                  $("#modal-default").modal({backdrop: false});
                  $('.modal-title').text('<?php echo label("editarea"); ?>');
                  $('#area_form')[0].reset();
                  $('#operation').val("Edit");

                  $('#salearea_code').attr('readonly', true);
                $('#salearea_id').val(data.salearea_id);
                $('#salearea_code').val(data.salearea_code);
                $('#salearea_shot').val(data.salearea_shot);
                $('#salearea_name_en').val(data.salearea_name_en);
                $('#salearea_name_th').val(data.salearea_name_th);
                $('#salearea_remark').val(data.salearea_remark);
                
                $.ajax({
                      url: '<?=base_url()?>index.php/querydata/rechecksection',
                      type: 'POST',
                      data:{section_id:data.section_id,com_id:com_id},
                      success: function(data_group){
                        $('#section_id').html(data_group);

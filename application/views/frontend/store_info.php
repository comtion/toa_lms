                ...
                <div class="row col-12 page-titles" id="div_maincourse">
                  <div class="col-md-12 card">
                    <div class="card-body">
                      <div class="col-md-12">
                        <?php if($btn_add=="1"){ ?>
                          <button name="add_button" id="add_button" class="btn btn-outline-info add_button float-right" data-toggle="modal" data-target="#modal-default"><i class="mdi mdi-plus-box-outline"></i> <?php echo label('addstore'); ?></button>
                        <?php } ?>
                          ...
                          <table id="myTable" width="100%" class="table table-bordered table-striped">
                            <thead>
                              <tr>
                                  ...
                                  <th width="5%"></th>
                                  <th width="10%"><center><?php echo label('store_code'); ?></center></th>
                                  <th width="10%"><center><?php echo label('store_cus_code'); ?></center></th>
                                  <th width="10%"><center><?php echo label('store_group'); ?></center></th>
                                  <th width="25%"><center><?php echo label('store_name'); ?></center></th>
              ...
              <div class="modal-body row">
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="st_code"><b style="color: #FF2D00">*</b><?php echo label('store_code'); ?>:</label>
                    <input type="text" id="st_code" name="st_code" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="st_cus_code"><b style="color: #FF2D00">*</b><?php echo label('store_cus_code'); ?>:</label>
                    <input type="text" id="st_cus_code" name="st_cus_code" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="st_group"><b style="color: #FF2D00">*</b><?php echo label('store_group'); ?>:</label>
                    <input type="text" id="st_group" name="st_group" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="st_name_th"><b style="color: #FF2D00">*</b><?php echo label('store_name')." TH"; ?>:</label>
                    <input type="text" id="st_name_th" name="st_name_th" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="st_name_en"><b style="color: #FF2D00">*</b><?php echo label('store_name')." EN"; ?>:</label>
                    <input type="text" id="st_name_en" name="st_name_en" class="form-control" required> 
                  </div>
                </div>
              </div>
      ...
      function fetch_data_main(page_num)
         {
            ...
            "scrollX": true,
                "ajax": {
                    url : '<?=base_url()?>index.php/manage/fetch_detail_store/',
                    type : 'GET',
                    data : {com_id:com_id}
                },
    ...
    $(document).ready(function() {
        ...
        $(document).on('submit', '#level_form', function(event){
              event.preventDefault(); 
                $.ajax({
                  url:"<?=base_url()?>index.php/manage/insert_store",
                  method:'POST',
         ...
         $(document).on('click', '.delete', function(){
            ...
            swal({
              ...
              if(isChk.value){
                $.ajax({
                    url:"<?=base_url()?>index.php/manage/delete_store_data",
                    method:"POST",
                    data:{st_id:st_id},
                    success:function(data)
                    {
           ...
           $('#add_button').click(function(){
                  ...
                  $('.modal-title').text('<?php echo label("addstore"); ?>');
                  $('#level_form')[0].reset();
                  $('#operation').val("Add");
                  $('#com_id').val(com_id);
                  $('#st_code').prop('readonly', false);
            });
          ...
          $(document).on('click', '.update', function(){
            ...
            $.ajax({
              url:"<?=base_url()?>index.php/manage/update_store_data",
              method:"POST",
              data:{st_id:st_id},
              dataType:"json",
              success:function(data)
              {         
                  $("#modal-default").modal({backdrop: false});
                  $('.modal-title').text('<?php echo label("editstore"); ?>');
                  $('#level_form')[0].reset();
                  $('#operation').val("Edit");
                $('#st_id').val(data.st_id);
                $('#st_cus_code').val(data.st_cus_code);
                $('#st_code').val(data.st_code);
                $('#st_group').val(data.st_group);
                $('#st_name_en').val(data.st_name_en);
                $('#st_name_th').val(data.st_name_th);

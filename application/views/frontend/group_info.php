                ...
                <div class="row col-12 page-titles" id="div_maincourse">
                  <div class="col-md-12 card">
                    <div class="card-body">
                      <div class="col-md-12">
                        <?php if($btn_add=="1"){ ?>
                          <button name="add_button" id="add_button" class="btn btn-outline-info add_button float-right" data-toggle="modal" data-target="#modal-default"><i class="mdi mdi-plus-box-outline"></i> <?php echo label('addgroup'); ?></button>
                        <?php } ?>
                          ...
                          <table id="myTable" width="100%" class="table table-bordered table-striped">
                            <thead>
                              <tr>
                                  ...
                                  <th width="5%"></th>
                                  <th width="15%"><center><?php echo label('div_name'); ?></center></th>
                                  <th width="10%"><center><?php echo label('group_code'); ?></center></th>
                                  <th width="30%"><center><?php echo label('group_name'); ?></center></th>
    ...
    <div class="modal fade bs-example-modal-lg" id="modal-default" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
      <div class="modal-dialog modal-lg">
          <div class="modal-content">
              ...
              <form method="post" id="group_form" autocomplete="off" name="group_form" enctype="multipart/form-data"  class="form-horizontal" role="form">
              <div class="modal-body row">
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="group_code"><b style="color: #FF2D00">*</b><?php echo label('div_name'); ?>:</label>
                    <select class="form-control select2" id="div_id" name="div_id" onchange="onchkcode($('#group_code').val())" style="width: 100%"></select>
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="group_code"><b style="color: #FF2D00">*</b><?php echo label('group_code'); ?>:</label>
                    <input type="text" id="group_code" name="group_code" onchange="onchkcode(this.value)" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="group_name_th"><b style="color: #FF2D00">*</b><?php echo label('group_name')." TH"; ?>:</label>
                    <input type="text" id="group_name_th" name="group_name_th" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="group_name_en"><b style="color: #FF2D00">*</b><?php echo label('group_name')." EN"; ?>:</label>
                    <input type="text" id="group_name_en" name="group_name_en" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-12">
                  <div class="form-group">
                    <label><?php echo label('detail'); ?>:</label>
                    <textarea class="form-control" rows="8" id="group_remark" name="group_remark"></textarea>
                  </div>
                </div>
              </div>
    ...
    <script type="text/javascript">
           ...
           function onchkcode(value){
              ...
              $.ajax({
                ...
                method:"POST",
                data:{code:value,field_name:"LMS_GROUP",com_id:com_id,val_2:div_id},
                dataType:"json",
      ...
      function fetch_data_main(page_num)
         {
            ...
            "scrollX": true,
                "ajax": {
                    url : '<?=base_url()?>index.php/manage/fetch_detail_group/',
                    type : 'GET',
                    data : {com_id:com_id}
                },
    ...
    $(document).ready(function() {
        ...
        $(document).on('submit', '#group_form', function(event){
              event.preventDefault(); 
                $.ajax({
                  url:"<?=base_url()?>index.php/manage/insert_group",
                  method:'POST',
         ...
         $(document).on('click', '.delete', function(){
            ...
            swal({
              ...
              if(isChk.value){
                $.ajax({
                    url:"<?=base_url()?>index.php/manage/delete_group_data",
                    method:"POST",
           ...
           $('#add_button').click(function(){
                  ...
                  $('.modal-title').text('<?php echo label("addgroup"); ?>');
                  $('#group_form')[0].reset();
                  $('#operation').val("Add");
                  $('#com_id').val(com_id);
                  ...
                  $.ajax({
                        url: '<?=base_url()?>index.php/querydata/recheckdivision',
                        type: 'POST',
                        data:{div_id:'',com_id:com_id},
                        success: function(data_div){
                          $('#div_id').html(data_div);
                          $('#div_id').val($("#div_id option:first").val()).trigger('change');
                        }
                  });
            });
          ...
          $(document).on('click', '.update', function(){
            ...
            var com_id = $('#com_id_search').val();
            $.ajax({
              url:"<?=base_url()?>index.php/manage/update_group_data",
              method:"POST",
              data:{group_id:group_id},
              dataType:"json",
              success:function(data)
              {         
                  $("#modal-default").modal({backdrop: false});
                  $('.modal-title').text('<?php echo label("editgroup"); ?>');
                  $('#group_form')[0].reset();
                  $('#operation').val("Edit");
                ...
                $('#group_name_en').val(data.group_name_en);
                ...
                $.ajax({
                      url: '<?=base_url()?>index.php/querydata/recheckdivision',
                      type: 'POST',
                      data:{div_id:data.div_id,com_id:com_id},
                      success: function(data_div){
                        $('#div_id').html(data_div);

                ...
                <div class="row col-12 page-titles" id="div_maincourse">
                  <div class="col-md-12 card">
                    <div class="card-body">
                      <div class="col-md-12">
                        <?php if($btn_add=="1"){ ?>
                          <button name="add_button" id="add_button" class="btn btn-outline-info add_button float-right" data-toggle="modal" data-target="#modal-default"><i class="mdi mdi-plus-box-outline"></i> <?php echo label('adddepartment'); ?></button>
                        <?php } ?>
                          ...
                          <table id="myTable" width="100%" class="table table-bordered table-striped">
                            <thead>
                              <tr>
                                  ...
                                  <th width="5%"></th>
                                  <th width="20%"><center><?php echo label('group_name'); ?></center></th>
                                  <th width="10%"><center><?php echo label('dep_code'); ?></center></th>
                                  <th width="25%"><center><?php echo label('dep_name'); ?></center></th>
    ...
    <div class="modal fade bs-example-modal-lg" id="modal-default" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
      <div class="modal-dialog modal-lg">
          <div class="modal-content">
              ...
              <form method="post" id="dep_form" autocomplete="off" name="dep_form" enctype="multipart/form-data"  class="form-horizontal" role="form">
              <div class="modal-body row">
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="group_id"><b style="color: #FF2D00">*</b><?php echo label('group_name'); ?>:</label>
                    <select class="form-control select2" id="group_id" name="group_id" onchange="onchkcode($('#dep_code').val())" style="width: 100%"></select>
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="dep_code"><b style="color: #FF2D00">*</b><?php echo label('dep_code'); ?>:</label>
                    <input type="text" id="dep_code" name="dep_code" onchange="onchkcode(this.value)" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="dep_shot"><b style="color: #FF2D00">*</b><?php echo label('acronym'); ?>:</label>
                    <input type="text" id="dep_shot" name="dep_shot" class="form-control" required> 
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="form-group">
                ...
                <div class="col-md-12">
                  <div class="form-group">
                    ...
                    <textarea class="form-control" rows="8" id="dep_remark" name="dep_remark"></textarea>
                  </div>
                </div>
              </div>
    ...
    <script type="text/javascript">
           ...
           function onchkcode(value){
              ...
              $.ajax({
                ...
                method:"POST",
                data:{code:value,field_name:"LMS_DEPART",com_id:com_id,val_2:group_id},
                dataType:"json",
        ...
        $(document).on('submit', '#dep_form', function(event){
         ...
         $(document).on('click', '.delete', function(){
           ...
           $('#add_button').click(function(){
                  ...
                  $('.modal-title').text('<?php echo label("adddepartment"); ?>');
                  $('#dep_form')[0].reset();
                  $('#operation').val("Add");
                  $('#com_id').val(com_id);
                  $('#dep_code').attr('readonly', false);

                  $.ajax({
                        url: '<?=base_url()?>index.php/querydata/recheckgroup',
                        type: 'POST',
                        data:{group_id:'',com_id:com_id},
                        success: function(data_group){
                          $('#group_id').html(data_group);
                          $('#group_id').val($("#group_id option:first").val()).trigger('change');
                        }
                  });
            });
          ...
          $(document).on('click', '.update', function(){
            ...
            $.ajax({
              ...
              {         
                  $("#modal-default").modal({backdrop: false});
                  $('.modal-title').text('<?php echo label("editdepartment"); ?>');
                  $('#dep_form')[0].reset();
                  $('#operation').val("Edit");
                  $('#dep_code').attr('readonly', true);

                $('#dep_id').val(data.dep_id);
                $('#dep_code').val(data.dep_code);
                $('#dep_shot').val(data.dep_shot);
                ...
                $.ajax({
                      url: '<?=base_url()?>index.php/querydata/recheckgroup',
                      type: 'POST',
                      data:{group_id:data.group_id,com_id:com_id},
                      success: function(data_group){
                        $('#group_id').html(data_group);

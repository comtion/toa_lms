 var i_emp, i_user, i_pass, i_cpass, emp_c, useri;

$(document).ready(function() {
      checkInitInput();
      
      $('[name="emp"]').blur(function() {
        if(!$('[name="emp"]').prop('readonly')) {
          checkEmp($(this).val());
        }
      });
      $('[name="user"]').blur(function() {
       /* if(!$('[name="user"]').prop('readonly')) {
          checkUser($(this).val());
        }*/
      });
...
function checkInitInput() {
  if($('[name="emp"]').val() !== '') i_emp = true;
  if($('[name="user"]').val() !== '') i_user = true;
  emp_c = $('[name="emp"]').val();
  useri = $('[name="user"]').val();
  checkInput();
}
...
function checkInput() {
  if(i_user) $('#user [name="done"]').prop('disabled', false);
  else $('#user [name="done"]').prop('disabled', true);
  if(i_pass && i_cpass) $('#pass [name="done"]').prop('disabled', false);
  else $('#pass [name="done"]').prop('disabled', true);
}
